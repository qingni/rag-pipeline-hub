...
| semantic | embedding_model | bge-m3/qwen3-embedding-8b/hunyuan-embedding | bge-m3 |
| parent_child | parent_size | 500-10000 | 2000 |
| parent_child | child_size | 100-2000 | 500 |
| parent_child | child_overlap | 0-200 | 50 |
| hybrid | embedding_model | bge-m3/qwen3-embedding-8b/hunyuan-embedding | bge-m3 |
...
| hunyuan-embedding | 1024 | 1K | 腾讯混元 | 腾讯生态集成 |
...
# 语义分块默认 Embedding 模型
# 可选: bge-m3 (推荐) / qwen3-embedding-8b (高精度) / hunyuan-embedding
...
### Q: 语义分块算法升级后有什么变化？
A: 升级为基于统一 EmbeddingService 的 Embedding 相似度，支持 bge-m3（推荐）、qwen3-embedding-8b（高精度）、hunyuan-embedding 三种模型。如果 Embedding 服务不可用，自动降级为 TF-IDF，最终降级为句子累加。

### Q: 如何选择 Embedding 模型？
A: 
- **bge-m3（推荐默认）**：1024维，8K上下文，多语言支持，速度快，适合大多数场景
- **qwen3-embedding-8b**：4096维，32K上下文，高精度，适合精度敏感或超长文本场景
- **hunyuan-embedding**：1024维，腾讯混元，适合腾讯生态集成场景

...
### 4.4 多 Collection 上下文合并

当从多个 Collection 检索时，需要合并结果：

```python
def merge_context_from_collections(results_list: List[List[dict]], top_k: int) -> List[ContextItem]:
    """合并多个 Collection 的检索结果"""
    all_results = []
...
3. **长度控制**: 基于 token 预算做整体截断，而不是固定字符数
4. **来源多样性**: 尽量包含不同来源的信息
5. **顺序保持**: 默认信任搜索模块返回的相关性顺序，按顺序累加直到预算耗尽
...
1. **流式输出**: 后端使用 SSE 格式返回，前端用 `fetch` 流式解析，提升用户体验
2. **Token估算**: 提前估算 Token 数，避免超限
3. **超时控制**: 设置合理的请求超时时间
4. **重试机制**: SDK 内置重试，前端提供显式“重试”入口
...
2. **敏感词过滤**: 过滤敏感内容
3. **输出审核**: 检查生成内容的合规性
4. **结果清洗**: 前端对 Markdown 渲染结果做 HTML 安全清洗
5. **日志记录**: 记录请求和响应用于审计

...
5. [Qwen3 VL Embedding 多模态模型](#5-qwen3-vl-embedding-多模态模型)
6. [模型对比与选择](#6-模型对比与选择)
7. [配置与使用](#7-配置与使用)
...
| 模型 | 维度 | 提供商 | 类型 | 特点 |
|------|------|--------|------|------|
| bge-m3 | 1024 | BAAI/SiliconFlow | 文本 | 多语言、多任务、支持密集/稀疏/多向量检索 |
| qwen3-embedding-8b | 4096 | 阿里云/SiliconFlow | 文本 | 高维度、强语义、支持32K长文本 |
| qwen3-vl-embedding-8b | 64-4096 | 阿里云/SiliconFlow | 多模态 | 支持文本、图像、视频输入 |
...
## 5. Qwen3 VL Embedding 多模态模型

### 5.1 模型介绍

Qwen3-VL-Embedding-8B 是阿里云通义千问团队开发的多模态嵌入模型，支持文本、图像、视频及任意多模态组合输入。

### 5.2 核心特点

- **多模态支持**: 支持文本、图像、截图、视频及任意多模态组合输入
- **动态维度**: 支持自定义输出维度 (64 至 4096)
- **中英双语**: 对中文和英文都有良好支持
- **统一语义空间**: 在统一语义空间中生成文本和视觉内容的向量表示
- **支持指令**: 支持instruction prefix提升检索效果

### 5.3 技术参数

| 参数 | 值 |
|------|-----|
| 向量维度 | 64-4096 (可配置，默认4096) |
| 最大序列长度 | 32768 tokens |
| 支持语言 | 中文、英文 |
| 模型类型 | multimodal |
| 支持输入 | text、image、video、text+image |
...
| 模型 | 维度 | 最大序列长度 | 速度 | 中文 | 英文 | 多模态 |
|------|------|-------------|------|------|------|--------|
| bge-m3 | 1024 | 8192 | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ | ⭐ |
| qwen3-embedding-8b | 4096 | 32768 | ⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐ |
| qwen3-vl-embedding-8b | 64-4096 | 32768 | ⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ |
...
| qwen3-vl-embedding-8b | 4096 (默认) | ~16KB | ~160MB | ~16GB |

> 注: qwen3-vl-embedding-8b 支持动态维度配置 (64-4096)，存储空间根据实际配置维度而定
### 6.3 选择指南
...
#### 场景5: 企业内部部署（数据安全要求高）
**推荐**: bge-m3 或 qwen3-embedding-8b
- 理由: 支持通过私有化部署满足数据安全要求

#### 场景6: 多模态图文检索
**推荐**: qwen3-vl-embedding-8b
- 理由: 当前唯一支持多模态输入的模型
...
```bash
# .env 文件
SILICONFLOW_API_KEY=your_api_key  # 用于 BGE-M3, Qwen3 系列模型
HUNYUAN_API_KEY=your_api_key      # 用于腾讯混元模型
```
